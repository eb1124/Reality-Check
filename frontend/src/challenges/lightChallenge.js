...
export function evaluateLightResponse({
  ...
  if (isSaturated(baselineCombined.mean, config) || isSaturated(responseCombined.mean, config)) {
    return { outcome: 'INCONCLUSIVE', reason: 'ROI pixel values saturated/clipped during baseline or response window — measurement unreliable.', details };
  }

  if (baselineCombined.mean.luminance < config.minAmbientLuminance) {
    return { outcome: 'INCONCLUSIVE', reason: 'Ambient (pre-flash) luminance below the minimum trustworthy floor.', details };
  }
  ...
  if (!directionOk) {
    return { outcome: 'INCONCLUSIVE', reason: 'No colour/chromaticity shift in the expected direction for this flash colour.', details };
  }
  ...
  if (Math.abs(chromaDelta) <= noiseFloor) {
    return { outcome: 'INCONCLUSIVE', reason: 'Response delta did not exceed the baseline noise floor.', details };
  }
  ...
  if (!bilateralConsistent) {
    return { outcome: 'INCONCLUSIVE', reason: 'Left/right cheek response inconsistent (asymmetric — possible localized artifact rather than global skin response).', details };
  }
  ...
  if (globalDriftSuspect) {
    return { outcome: 'INCONCLUSIVE', reason: 'Full-frame reference shows a large simultaneous shift — possible auto-exposure/white-balance compensation dominating the local signal.', details };
  }

  if (Math.abs(chromaDelta) <= confidentThreshold) {
    return { outcome: 'INCONCLUSIVE', reason: 'Directionally-correct response observed but below the confident-pass magnitude.', details };
  }

  return { outcome: 'PASS', reason: 'Directionally-consistent, bilateral, above-noise-floor colour/chromaticity response observed while the flash was active.', details };
}
